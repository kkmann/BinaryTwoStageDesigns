...
#     zmodprior = quadgk(modprior_, l, u)[1]
#     modprior(p) = modprior_(p) / zmodprior * (p < l ? 0 : (l - u))
#     zprior = quadgk(prior, l, u)[1]
#     cprior(p) = prior(p) / zprior * (p < l ? 0 : (l - u))
#     cdfrelaxed = cumsum(modprior.(candidates))
#     cdfrelaxed = cdfrelaxed / cdfrelaxed[resolution]
#     cdf = cumsum(cprior.(candidates))
...
function findgrid(prior, l, u, n)
    ...
    zprior = quadgk(prior, l, u)[1]
    modcdf(p) = quadgk(modprior, l, p)[1]
    cprior(p) = prior(p) / zprior * (p < l ? 0 : 1)
    cdf(p) = quadgk(cprior, l, p)[1]
    pivots  = [l + 10^-6.0]
    modcdfs = [0.0]
    cdfs    = [0.0]
    for i in 2:(n + 1)
